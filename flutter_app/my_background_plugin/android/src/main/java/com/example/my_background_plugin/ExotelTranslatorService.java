...
public class ExotelTranslatorService extends Service implements ExotelVoiceClientEventListener, CallListener {
    ...
    private MethodChannel channel = ChannelManager.getChannel();
...
//            channel = new MethodChannel(messenger, "my_background_plugin"); // Replace "your_channel_name" with your actual channel name
//
//                  } else {
...
//                .setSmallIcon(R.drawable.ic_service_icon)
    ...
    public void onMethodCall(MethodCall call, Result result) {
        System.out.println("in exotel Context initialized: " + (context != null) + channel);
    ...
    public void onInitializationSuccess() {
        ...
        if (channel != null) {
            VoiceAppLogger.error(TAG, "Channel is not null" + channel);
            uiThreadHandler.post(()->{
                channel.invokeMethod("on-inialization-success", null);
            });
        } else {
    ...
    private void sendMessageToFlutter() {
        ...
        channel.invokeMethod("receiveMessage", "Hello from Java");
    }
